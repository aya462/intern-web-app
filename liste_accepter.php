...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM stagiere WHERE statut='accepter'");
    $stmt->execute();
...
<body>
    <div class="container">
        <h2>Liste des stagiaires acceptés</h2>
        <table>
            <thead>
                <tr>
                <th>ID</th>
                    ...
                    <th>message </th>
                    <th>Évalué</th>
                    <th>Evaluer le stage</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($result as $row) {
                   // Initialisation d'une variable pour vérifier si une évaluation existe pour ce stage
                    $evaluated = false;
                    // Vérifier si une évaluation existe pour ce stage
                    $stmt = $conn->prepare("SELECT * FROM evaluations WHERE intern_id = :intern_id");
                    $stmt->bindParam(':intern_id', $row['id']);
                    $stmt->execute();
                    $evaluation = $stmt->fetch(PDO::FETCH_ASSOC);
                     // Si une évaluation existe, mettre la variable $evaluated à true
                    if ($evaluation) {
                        $evaluated = true;
                    }
                ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['message']) ?></td>
                          <!-- Si évalué, afficher une icône de vérification -->
                        <td><?= $evaluated ? '<i class="fas fa-check text-success"></i>' : '' ?></td>
                        <td><a href="evaluateStage.php?intern_id=<?= htmlspecialchars($row['id']) ?>&intern_name=<?= htmlspecialchars($row['NomPrenom']) ?>"><i class="fas fa-star"></i></a></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
